...
def cor_do_pokemon(nome):
    ...
    if retorno.status_code != 200:
        ...
        pokemon = retorno.json()
        cor_ingles = pokemon["color"]["name"]
        for cor in dic_cores:
            if cor == cor_ingles:
                return dic_cores.get(cor)
...
def tipos_do_pokemon(nome):
    ...
    if retorno.status_code != 200:
        ...
        for tipo in pokemon["types"]:
            tipos_lista_ingles.append(tipo["type"]["name"])
        tipos_lista_pt = []
        for tipo in dic_tipos:
            if tipo in tipos_lista_ingles:
                tipos_lista_pt.append(dic_tipos.get(tipo))
        return tipos_lista_pt
...
def nivel_do_pokemon(nome, experiencia):
    ...
    if retorno.status_code != 200:
        ...
        pokemon = retorno.json()
        url_growth = requests.get(pokemon["growth_rate"]["url"])
        json_growth = url_growth.json()

        if experiencia >= json_growth["levels"][-1]["experience"]:
            return json_growth["levels"][-1]["level"]
        else:
            for indice, level in enumerate(json_growth["levels"]):
                if (
                    experiencia >= level["experience"]
                    and experiencia < json_growth["levels"][indice + 1]["experience"]
                ):
                    return level["level"]
