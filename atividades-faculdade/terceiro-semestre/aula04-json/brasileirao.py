...
def qtos_libertadores(dados):
    classificados = dados["fases"]["2700"]["faixas-classificacao"]["classifica1"][
        "faixa"
    ]
    return int(classificados.split("-")[1])
...
def ids_dos_melhor_classificados(dados, numero_de_times):
    ...
    while contador < numero_de_times:
        lista_times.append(
            dados["fases"]["2700"]["classificacao"]["grupo"]["Único"][contador]
        )
        contador += 1
    return lista_times
...
def nomes_classificados_libertadores(dados):
    ...
    for time in classificados_libertadores(dados):
        lista_nomes.append(dados["equipes"][time]["nome-comum"])
    return lista_nomes
...
def nomes_dos_times_de_um_jogo(dados, id_jogo):
    nomes = []
    for id_time in ids_dos_times_de_um_jogo(dados, id_jogo):
        nomes.append(nome_do_time(dados, id_time))
    return nomes
...
def id_do_time(dados, nome_time):
    for equipe in dados["equipes"]:
        if dados["equipes"][equipe]["nome-comum"] == nome_time:
            return dados["equipes"][equipe]["id"]
...
def dicionario_id_estadio_e_nro_jogos(dados):
    ...
    for jogo in dados["fases"]["2700"]["jogos"]["id"]:
        if dados["fases"]["2700"]["jogos"]["id"][jogo]["estadio_id"] in jogosEstadios:
            jogosEstadios[
                dados["fases"]["2700"]["jogos"]["id"][jogo]["estadio_id"]
            ] += 1
        else:
            jogosEstadios[dados["fases"]["2700"]["jogos"]["id"][jogo]["estadio_id"]] = 1
    return jogosEstadios
...
def busca_imprecisa_por_nome_de_time(dados, nome_time):
    ...
    for time in dados["equipes"]:
        if (
            nome_time in dados["equipes"][time]["nome"]
            or nome_time in dados["equipes"][time]["nome-comum"]
            or nome_time in dados["equipes"][time]["nome-slug"]
            or nome_time in dados["equipes"][time]["sigla"]
        ):
            listaIds.append(time)
    return listaIds
...
def ids_de_jogos_de_um_time(dados, time_id):
    ...
    for jogo in dados["fases"]["2700"]["jogos"]["id"]:
        if (
            time_id in dados["fases"]["2700"]["jogos"]["id"][jogo]["time1"]
            or time_id in dados["fases"]["2700"]["jogos"]["id"][jogo]["time2"]
        ):
            idJogos.append(jogo)
    return idJogos
...
def datas_de_jogos_de_um_time(dados, nome_time):
    idsJogos = ids_de_jogos_de_um_time(dados, id_do_time(dados, nome_time))
    listaDatas = []
    for jogo in dados["fases"]["2700"]["jogos"]["id"]:
        if jogo in idsJogos:
            listaDatas.append(dados["fases"]["2700"]["jogos"]["id"][jogo]["data"])
    return listaDatas
...
def dicionario_de_gols(dados):
    golsTimes = {}
    for jogo in dados["fases"]["2700"]["jogos"]["id"]:
        if dados["fases"]["2700"]["jogos"]["id"][jogo]["time1"] not in golsTimes:
            golsTimes[dados["fases"]["2700"]["jogos"]["id"][jogo]["time1"]] = int(
                dados["fases"]["2700"]["jogos"]["id"][jogo]["placar1"]
            )
        else:
            golsTimes[dados["fases"]["2700"]["jogos"]["id"][jogo]["time1"]] += int(
                dados["fases"]["2700"]["jogos"]["id"][jogo]["placar1"]
            )
        if dados["fases"]["2700"]["jogos"]["id"][jogo]["time2"] not in golsTimes:
            golsTimes[dados["fases"]["2700"]["jogos"]["id"][jogo]["time2"]] = int(
                dados["fases"]["2700"]["jogos"]["id"][jogo]["placar2"]
            )
        else:
            golsTimes[dados["fases"]["2700"]["jogos"]["id"][jogo]["time2"]] += int(
                dados["fases"]["2700"]["jogos"]["id"][jogo]["placar2"]
...
def time_que_fez_mais_gols(dados):
    idMaior = 0
    qtdGol = 0
    dicGols = dicionario_de_gols(dados)
    for idTime in dicionario_de_gols(dados):
        if dicGols[idTime] > qtdGol:
            idMaior = idTime
            qtdGol = dicGols[idTime]
    return idMaior
...
def rebaixados(dados):
    ...
    inicio = int(rebaixados.split("-")[0])
    final = int(rebaixados.split("-")[1])

    timesRebaixados = []
    while inicio <= final:
        timesRebaixados.append(
            dados["fases"]["2700"]["classificacao"]["grupo"]["Único"][inicio - 1]
        )
        inicio += 1
    return timesRebaixados
...
def classificacao_do_time_por_id(dados, time_id):
    classificacao = 1
    for posicao in dados["fases"]["2700"]["classificacao"]["grupo"]["Único"]:
        if posicao == time_id:
...
class TestClientes(unittest.TestCase):
    ...
    def test_004_ids_dos_melhor_classificados(self):
        dados = pega_dados()
        self.assertEqual(
            ids_dos_melhor_classificados(dados, 10),
            ["17", "1", "15", "13", "24", "4", "3", "9", "5", "22"],
        )
        self.assertEqual(
            ids_dos_melhor_classificados(dados, 5), ["17", "1", "15", "13", "24"]
        )
    ...
    def test_005_classificados_libertadores(self):
        dados = pega_dados()
        self.assertEqual(
            classificados_libertadores(dados), ["17", "1", "15", "13", "24", "4"]
        )
        # vou falsificar os dados pra testar se vc esta lendo direito da estrutura
        dados["fases"]["2700"]["faixas-classificacao"]["classifica1"]["faixa"] = "1-8"
        self.assertEqual(
