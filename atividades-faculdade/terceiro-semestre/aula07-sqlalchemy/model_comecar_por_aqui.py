...
def mensagem_de_ataque_fisico(dano, nome_atacante, nome_defensor):
    msgs = [
        f"{nome_atacante} dá um soco em {nome_defensor}, causando {dano} de dano",
...
def mensagem_de_ataque_magico(dano, nome_atacante, nome_defensor):
    msgs = [
        f"{nome_atacante} solta raios contra {nome_defensor}, causando {dano} de dano",
...
class TestStringMethods(unittest.TestCase):
    ...
    def test_ex24_dar_item_para_heroi_e_colocar_item_em_uso(self):
        itens.criar_item(
            tipo="espada", nome="celestial", fisico=3, agilidade=3, magia=3
        )
        chun = heroi_pronto_por_nome("chun-li")
    ...
    def test_ex25_heroi_nao_pode_usar_dois_itens_do_mesmo_tipo(self):
        ...
        self.assertRaises(
            HeroiJaUsaEsseTipoDeItemException, colocar_item_em_uso, chun, vorpal
        )
